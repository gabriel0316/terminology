> **Für die deutsche Version bitte [hier](setup_de.html) klicken.**

### Quickstart

Please refer to the [Quickstart](quickstart_en.html) for setting up your own TerminoloGit.
...
Refer to the description of [how to delete a terminology](use_cases_en.html#delete-a-terminology) in order to reset your TerminoloGit fork.
...
| `convertFHIRServer` | URL of the FHIR® server's endpoint to be used for converting between FHIR® XML and FHIR® JSON. | **yes** | `https://myfhirserver/r4` | `Variable` | `All` | no | no | no |
| `convertFHIRServerTergiTunnelToken` | When using a [tergi](https://gitlab.com/elga-gmbh/terminologit-tergi) you can specify a token which allows to access the FHIR® server. In this case `convertFHIRServerUser` and `convertFHIRServerUserPassword` are not required. | no | `thisisasampletunneltoken` | `Variable` | `All` | no | **yes** | no |
| `convertFHIRServerUser` | Username for accessing the FHIR® server. | Yes, if `convertFHIRServerUserPassword` is used. No, if `convertFHIRServerTergiTunnelToken` is used. | `sampleuser` | `Variable` | `All` | no | no | no |
| `convertFHIRServerUserPassword` | Password of the corresponding `convertFHIRServerUser` for accessing the FHIR® server. | Yes, if `convertFHIRServerUser` is used. No, if `convertFHIRServerTergiTunnelToken` is used. | `sampelpassword` | `Variable` | `All` | no | **yes** | no |
...
| `UPLOAD_ARTIFACTS` | If set to `true` and the pipeline is run on the default branch the terminologies converted by MaLaC-CT and those within `input/resources/` will be uploaded to the FHIR® server which is used for converting between FHIR® XML and FHIR® JSON (see `convertFHIRServer`). | no | `true` | `Variable` | `All` | no | no | no |
| `UPLOAD_ARTIFACTS_ALL_BRANCHES` | If set to `true` regardless of the branch the terminologies converted by MaLaC-CT and those within `input/resources/` will be uploaded to the FHIR® server which is used for converting between FHIR® XML and FHIR® JSON (see `convertFHIRServer`). | no | `true` | `Variable` | `All` | no | no | no |
...
| `CI_COMMIT_TITLE` | The full first line of the commit message, also known as commit title. Commit messages which start with `AUTOMATIC_COMMIT` shall not trigger a pipeline. Refer to the chapter explaining how to [start a pipeline](use_cases_en.html#start-a-pipeline) to see how a pipeline can be triggered without commit. | `Add terminology` |
...
| `CS_INPUT_FORMAT` | [File format](file_formats_en.html) to be used as source for code systems by MaLaC-CT for creating all other file formats. | **Yes**, if the pipeline has not been started within the MaLaC-CT project. | `.1.propcsv.xlsx` | `Variable` | `All` | no | no | no |
| `VS_INPUT_FORMAT` | [File format](file_formats_en.html) to be used as source for value sets by MaLaC-CT for creating all other file formats. | **Yes**, if the pipeline has not been started within the MaLaC-CT project. | `.1.propcsv.xlsx` | `Variable` | `All` | no | no | no |
