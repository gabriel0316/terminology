> **For the english version please click [here](setup_en.html).**

### Quickstart

Bitte lesen Sie den [Quickstart](quickstart_de.html), um Ihr eigenes TerminoloGit einzurichten.
...
Lesen Sie die Beschreibung über das [Löschen einer Terminologie](use_cases_de.html#löschen-einer-terminologie), um Ihren TerminoloGit Fork zurückzusetzen.
...
| `convertFHIRServer` | URL des Endpunkts des FHIR®-Servers, der für die Konvertierung zwischen FHIR® XML und FHIR® JSON verwendet werden soll. | **ja** | `https://meinfhirserver/r4` | `Variable` | `Alle` | nein | nein | nein |
| `convertFHIRServerTergiTunnelToken` | Bei der Verwendung eines [tergi](https://gitlab.com/elga-gmbh/terminologit-tergi) können Sie ein Token angeben, das den Zugriff auf den FHIR®-Server ermöglicht. In diesem Fall sind `convertFHIRServerUser` und `convertFHIRServerUserPassword` nicht erforderlich. | nein | `dasisteinbeispieltoken` | `Variable` | `Alle` | nein | **ja** | nein |
| `convertFHIRServerUser` | Benutzername für den Zugriff auf den FHIR®-Server. | Ja, wenn `convertFHIRServerUserPassword` verwendet wird. Nein, wenn `convertFHIRServerTergiTunnelToken` verwendet wird. | `beispielbenutzer` | `Variable` | `Alle` | nein | nein | nein |
| `convertFHIRServerUserPassword` | Passwort des entsprechenden `convertFHIRServerUser` für den Zugriff auf den FHIR®-Server. | Ja, wenn `convertFHIRServerUser` verwendet wird. Nein, wenn `convertFHIRServerTergiTunnelToken` verwendet wird. | `beispielpasswort` | `Variable` | `Alle` | nein | **ja** | nein |
...
| `VS_INPUT_FORMAT` | [Dateiformat](file_formats_de.html), das von MaLaC-CT als Quelle für Value-Sets zur Erstellung aller anderen Dateiformate verwendet werden soll. | **Ja**, wenn die Pipeline nicht innerhalb des MaLaC-CT-Projekts gestartet wurde. | `.1.propcsv.xlsx` | `Variable` | `Alle` | nein | nein | nein |
